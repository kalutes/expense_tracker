...
export function TransactionLedger({ accounts, transactions }: TransactionLedgerProps) {
  ...
  const filteredTransactions = useMemo(() => {
    ...
    return enrichedTransactions.filter((t) => {
      ...
      if (query) {
        ...
        const rawMatch = t.parsedDetails.rawMerchant?.toLowerCase().includes(query) || t.parsedDetails.rawDesc?.toLowerCase().includes(query);
  ...
  const handleExportCsv = () => {
    ...
    const headers = ['Date', 'Account', 'Currency', 'Type', 'Amount', 'Merchant', 'User', 'FX Amount', 'FX Currency', 'FX Rate', 'Status', 'External ID'];
  ...
  return (
    <div className="space-y-4 font-mono">
        ...
        <div className="flex items-center gap-2 text-xs">
          {hasActiveFilters && (
            <button
              ...
              className="px-2.5 py-1 text-slate-300 hover:text-white bg-[#161b22] hover:bg-[#21262d] border border-[#30363d] rounded transition-colors cursor-pointer flex items-center gap-1"
            >
          ...
          <button
            ...
            className="px-3 py-1 text-emerald-400 hover:text-emerald-300 bg-[#161b22] hover:bg-[#21262d] border border-emerald-500/40 rounded transition-colors cursor-pointer flex items-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed"
          >
      ...
      <div className="bg-[#0d1117] border border-[#30363d] rounded-xl p-4 space-y-3">
        {/* Row 1: Search Input & Dynamic User Selector */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-3">
          ...
          <div className="lg:col-span-7 relative flex items-center">
            ...
            <input
              ...
              placeholder="filter query (e.g. Taco Bell, Costco, Amazon, 9.78)..."
              className="w-full pl-8 pr-8 py-1.5 bg-[#090d13] border border-[#30363d] focus:border-emerald-500 rounded text-xs text-white placeholder-slate-600 transition-colors outline-none font-mono"
            />
            {searchQuery && (
              <button
                onClick={() => {
                ...
                className="absolute right-2.5 text-slate-500 hover:text-white text-xs font-mono"
              >
                [x]
              </button>
            )}
          ...
          {availableUsers.length > 0 && (
            <div className="lg:col-span-5 flex items-center gap-1 p-1 bg-[#090d13] border border-[#30363d] rounded text-xs overflow-x-auto">
              ...
              <button
                ...
                className={`py-0.5 px-2 rounded text-[11px] transition-all whitespace-nowrap ${
                  selectedUser === 'ALL'
              ...
              {availableUsers.map((u) => {
                ...
                return (
                  <button
                    ...
                    className={`py-0.5 px-2 rounded text-[11px] transition-all flex items-center justify-center gap-1 whitespace-nowrap ${
                      isSelected
          ...
          {accounts.map((acc) => {
            ...
            return (
              <button
                ...
                className={`px-2 py-0.5 rounded text-[11px] transition-all border truncate max-w-[200px] ${
                  isSelected
        ...
        <div className="pt-2 border-t border-[#21262d] flex flex-wrap items-center justify-between gap-3 text-xs">
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            <div className="flex items-center gap-1 bg-[#090d13] border border-[#21262d] rounded p-0.5 text-[11px]">
              ...
              <button
                ...
                className={`px-1.5 py-0.5 rounded ${
                  selectedCurrency === 'USD' ? 'bg-emerald-950/60 text-emerald-400 border border-emerald-500/30 font-bold' : 'text-slate-500 hover:text-slate-300'
                }`}
              >
                $ USD
              </button>
              <button
                ...
                className={`px-1.5 py-0.5 rounded ${
                  selectedCurrency === 'JPY' ? 'bg-indigo-950/60 text-indigo-400 border border-indigo-500/30 font-bold' : 'text-slate-500 hover:text-slate-300'
                }`}
              >
                ¥ JPY
              </button>
            ...
            <div className="flex items-center gap-1 bg-[#090d13] border border-[#21262d] rounded p-0.5 text-[11px]">
              ...
              <button
                ...
                className={`px-1.5 py-0.5 rounded ${
                  selectedType === 'EXPENSE' ? 'bg-rose-950/60 text-rose-400 border border-rose-500/30 font-bold' : 'text-slate-500 hover:text-slate-300'
                }`}
              >
                - EXPENSES
              </button>
              <button
                ...
                className={`px-1.5 py-0.5 rounded ${
                  selectedType === 'INCOME' ? 'bg-emerald-950/60 text-emerald-400 border border-emerald-500/30 font-bold' : 'text-slate-500 hover:text-slate-300'
                }`}
              >
                + INFLOWS
              </button>
            ...
            <div className="flex items-center gap-1 text-[11px]">
              ...
              <select
                ...
                className="bg-[#090d13] border border-[#21262d] text-slate-300 rounded px-1.5 py-0.5 text-[11px] outline-none focus:border-emerald-500"
              >
          ...
          <div className="flex items-center gap-1.5 text-[11px]">
            ...
            <button
              ...
              className="px-1.5 py-0.5 bg-[#090d13] hover:bg-[#161b22] border border-[#21262d] rounded text-slate-300 hover:text-white"
            >
      ...
      {/* Live Dynamic Subtotals Banner - Flat Minimal */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 bg-[#0d1117] border border-[#21262d] rounded-lg p-3 text-xs">
        <div>
      ...
      <div className="bg-[#0d1117] border border-[#30363d] rounded-xl overflow-hidden">
        {paginatedTransactions.length === 0 ? (
          <div className="p-8 text-center text-xs text-slate-400 space-y-2">
            ...
            <button
              onClick={clearAllFilters}
              className="mt-2 px-3 py-1 text-xs text-emerald-400 bg-[#161b22] border border-[#30363d] rounded"
            >
              [RESET_ALL_FILTERS]
            </button>
          </div>
        ...
        <div className="px-3.5 py-2.5 border-t border-[#30363d] bg-[#090d13] flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 text-xs text-slate-400">
          <div className="flex items-center gap-2 text-[11px]">
            ...
            <select
              ...
              className="bg-[#161b22] border border-[#30363d] text-slate-300 rounded px-1.5 py-0.5 outline-none focus:border-emerald-500"
            >
          ...
          <div className="flex items-center gap-1.5 text-[11px]">
            <button
              ...
              className="px-2 py-0.5 rounded bg-[#161b22] border border-[#30363d] text-slate-300 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed"
            >
              |&lt;
            </button>
            <button
      ...
      {/* Flat Retro Inspector Modal */}
      {inspectTxn && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4">
          <div className="bg-[#0d1117] border border-[#30363d] rounded-xl max-w-xl w-full p-5 shadow-2xl space-y-3 max-h-[90vh] overflow-y-auto font-mono text-xs">
            <div className="flex items-start justify-between pb-2 border-b border-[#21262d]">
              <div>
                ...
                <h3 className="text-sm font-bold text-white mt-0.5">{inspectTxn.merchant}</h3>
              </div>
              <button
                onClick={() => setInspectTxn(null)}
                className="px-2 py-0.5 rounded text-slate-400 hover:text-white bg-[#161b22] border border-[#30363d]"
              >
                [ESC / CLOSE]
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div className="bg-[#090d13] p-2.5 rounded border border-[#21262d]">
                <span className="text-slate-500 block text-[10px]">AMOUNT</span>
                <span className={`text-sm font-bold ${inspectTxn.amount > 0 ? 'text-rose-400' : 'text-emerald-400'}`}>
            ...
            {/* Deduplication Hash */}
            <div className="bg-[#090d13] p-2.5 rounded border border-[#21262d] space-y-0.5">
              <span className="text-slate-500 block text-[10px]">DEDUP_HASH_SHA256</span>
              <span className="text-[10px] text-slate-400 break-all select-all block">
                {inspectTxn.dedupHash}
              </span>
            </div>

            {/* Raw Details JSON */}
            <div className="bg-[#090d13] p-2.5 rounded border border-[#21262d] space-y-1">
              <span className="text-slate-500 block text-[10px]">RAW_PAYLOAD_JSON</span>
              <pre className="text-[10px] text-emerald-300 overflow-x-auto p-2 bg-black rounded max-h-48">
            ...
            <div className="pt-1 flex justify-end">
              <button
                onClick={() => setInspectTxn(null)}
                className="px-3 py-1 text-xs text-white bg-[#161b22] hover:bg-[#21262d] border border-[#30363d] rounded"
              >
                [CLOSE]
              </button>
            </div>
